...
-- Generate a nice name map for printing
niceNameMap :: M.Map Name String
niceNameMap = M.fromList ((map Name [0..]) `zip` (gen 1 [])) where

    times :: Integer -> Char -> String
    times n c = case n of { 0 -> "" ; _ -> c : (n - 1) `times` c }

    gen :: Integer -> [String] -> [String]
    gen n ss = if n > 1000 then ss else
        gen (n + 1) (ss ++ (map (n `times`) ['a'..'z']))
